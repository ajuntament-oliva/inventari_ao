...
if (isset($_POST['add_owner'])) {
  ...
  $caracteristica = remove_junk($db->escape($_POST['caracteristica']));
  ...
  if ($check_department && $check_department->num_rows > 0) {
    if ($propietari_nom && $propietari_cognom && $dispositiu_nom && $caracteristica) {
      ...
      if ($db->query($sql_insert_owner)) {
        ...
        if ($db->query($sql_insert_device)) {
          ...
          if ($db->query($sql_insert_device_owner)) {
            ...
            $sql_insert_feature = "INSERT INTO caracteristiques_dispositiu (dispositiu_id, caracteristica) VALUES ($dispositiu_id, '$caracteristica')";
            if ($db->query($sql_insert_feature)) {
              $session->msg('s', "Propietari, dispositiu i característica afegits amb èxit.");
              redirect('add_dispositiu_detall.php?departament_id=' . $departament_id, false);
            } else {
              $session->msg('d', "No s'ha pogut afegir la característica: " . $db->error);
            }
          } else {
    ...
    <div class="panel panel-default">
      <div class="panel-body">
        ...
        <input type="hidden" name="departament_id" value="<?php echo $departament_id; ?>">
          ...
          <div class="form-group">
            <label for="caracteristica">Característiques Dispositiu:</label>
            <input type="text" name="caracteristica" class="form-control" required>
          </div>
