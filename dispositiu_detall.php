...
if (isset($_GET['id'])) {
    ...
    if ($dispositiu) {
        // Consultar BDA per obtindre les característiques de tots els dispositius del mateix tipus
        $caracteristiques = $db->query("SELECT c.uid, c.id_anydesck, c.processador, c.ram, c.capacitat, c.marca, c.dimensions, c.tipus, p.nom, p.cognom 
                                         ...
                                         WHERE d.dispositiu = ? 
                                         ORDER BY d.id", [$dispositiu['dispositiu']]);
    } else {
        $error_message = "Dispositiu no trobat.";
    }
} else {
    $error_message = "ID del dispositiu no especificat.";
}
    ...
    <div class="col-md-6">
        <div class="panel panel-default">
            <div class="panel-body">
                ...
                <?php endif; ?>
                <div class="mt-3">
                    <a href="view_departament.php?id=<?php echo (int) $dispositiu['departament_id']; ?>" class="btn btn-danger">Torna enrere</a>
                </div>
            </div>
        </div>
    </div>
